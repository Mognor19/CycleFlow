...
export default function inicio() {
  ...
  const cycleflowContext = useContext(CyclesFlowContext);
  const { addNewDiario, refreshDiario } = cycleflowContext;
  ...
  /*
  const nuevaDiario = async () => {
    if (diario) {
      await addNewDiario(diario, refreshDiario);
      navigation.goBack();
    } else {
      setErrorDiario(true);
    }
  };*/
  const handlerNewDiario = async () => {
